...
(function() {
  ...
  window.MP.player.MixerVoice = (function() {
    function MixerVoice(sampleRate) {
    ...
    MixerVoice.prototype.render = function(buffer, offset, samples) {
      ...
      for (i = _i = 0; 0 <= samples ? _i < samples : _i > samples; i = 0 <= samples ? ++_i : --_i) {
        this.pos += (3740000.0 / this.period) / this.sampleRate;
  ...
  window.MP.player.Mixer = (function() {
    Mixer.prototype.PAULARATE = 3740000.0;

    function Mixer(sampleRate) {
      var i, _i;
      this.sampleRate = sampleRate;
      this.voices = [];
      for (i = _i = 0; _i <= 3; i = ++_i) {
        this.voices.push(new window.MP.player.MixerVoice(this.sampleRate));
      }
  ...
  window.MP.player.Player = (function() {
    function Player() {
      this.js_render = __bind(this.js_render, this);
      this.load_from_json = __bind(this.load_from_json, this);
      var i, _i;
      this.module = null;
      this.channels = [];
      this.context = new AudioContext();
      this.OUTRATE = this.context.sampleRate;
      for (i = _i = 0; _i <= 3; i = ++_i) {
        this.channels.push(new window.MP.player.Channel(this));
      }
      this.mixer = new window.MP.player.Mixer(this.OUTRATE);
      this.calc_ptable();
      this.calc_vibtable();
      this.bpm = 125;
      this.reset();
      ...
      this.gain_rtl = this.context.createGain();
      this.gain_rtr = this.context.createGain();
      this.gain_ltl = this.context.createGain();
      this.gain_ltr = this.context.createGain();
      this.gain_rtl.gain.value = 1.0 - this.separation;
      this.gain_ltr.gain.value = 1.0 - this.separation;
      this.gain_rtr.gain.value = this.separation;
      this.gain_ltl.gain.value = this.separation;
      this.splitter.connect(this.gain_ltl, 0);
      this.splitter.connect(this.gain_ltr, 0);
      this.splitter.connect(this.gain_rtl, 1);
      this.splitter.connect(this.gain_rtr, 1);
      this.gain_rtl.connect(this.merger, 0, 0);
      this.gain_ltl.connect(this.merger, 0, 0);
      this.gain_ltr.connect(this.merger, 0, 1);
      this.gain_rtr.connect(this.merger, 0, 1);
    ...
    Player.prototype.prep_nodes = function() {
      console.log("PREP NODES");
      if (this.render_node == null) {
        this.render_node = this.context.createScriptProcessor(2048, 2, 2);
        this.render_node.connect(this.splitter);
        return this.render_node.onaudioprocess = this.js_render;
      }
    };

    Player.prototype.js_render = function(e) {
      var l_buf, length, r_buf;
      length = e.outputBuffer.getChannelData(0).length;
      l_buf = e.outputBuffer.getChannelData(0);
      r_buf = e.outputBuffer.getChannelData(1);
      return this.render(l_buf, r_buf, length);
    };
    ...
    Player.prototype.render = function(l_buf, r_buf, len) {
      var i, offset, todo, _i, _j, _results;
